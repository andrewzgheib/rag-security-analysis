...
def get_client() -> chromadb.PersistentClient:
    ...
    if not client:
        raise EnvironmentError(f"[!] Failed to initialize ChromaDB client with persistence at '{CHROMA_PERSIST_DIR}'. Please check config.py.")
...
def query_store( query: str, top_k: int = TOP_K, where: dict | None = None) -> list[dict]:
    ...
    kwargs: dict = dict(query_embeddings=[q_embedding], n_results=min(top_k, collection.count()), include=["documents", "distances", "metadatas"])
    if where:
        kwargs["where"] = where
    ...
    return [
        ...
        for doc, meta, dist, rid in zip(results["documents"][0], results["metadatas"][0], results["distances"][0], results["ids"][0])
    ]
...
def reset_collection() -> None:
    ...
    client.get_or_create_collection(name=CHROMA_COLLECTION_NAME, metadata={"hnsw:space": "cosine"})